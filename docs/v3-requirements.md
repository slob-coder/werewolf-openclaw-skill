...
| 编号 | 内容 | 优先级 | 预估工作量 | 依赖 |
|------|------|--------|-----------|------|
| F-01 | SDK 重构 Bridge 通信层 | P0 | 1.5d | SDK 可安装 |
| F-02 | 业务脚本 werewolf_cli.py | P0 | 1d | F-01 |
| F-03 | SKILL.md 重构 | P0 | 0.5d | F-02 |
| F-04 | Bridge 事件转发重构 | P0 | 1d | F-01 |
| BUG-01 | 女巫专有阶段 | P1 | 0.5d | NFR-03 |
| BUG-02 | 猎人开枪阶段 | P1 | 0.5d | NFR-03 |
| BUG-03 | role.assigned 事件 | P1 | 0.5d | NFR-03 |
| BUG-04 | night.result 事件 | P1 | 0.5d | NFR-03 |
| BUG-05 | 降级策略补全 | P2 | 0.5d | F-02 |
| NFR-01 | 依赖管理 | P0 | 0.5h | — |
| NFR-02 | 上下文传递方案 | P0 | 0.5h | F-02 |
| NFR-03 | 事件名确认 | P0 | 0.5d | — |
...
├── SKILL.md                  # [重写] 策略手册，不含 JSON 格式
├── werewolf_cli.py           # [新增] 业务脚本，高层命令 → SDK 调用
├── bridge.py                 # [重写] 基于 SDK WerewolfAgent 的 Bridge
├── README.md                 # [更新]
├── requirements.txt          # [更新] werewolf-arena + httpx
├── docs/
│   ├── design/
│   │   ├── v1-design.md
│   │   ├── v2-design.md
│   │   └── v3-design.md      # [新增] V3 设计文档
│   └── v3-requirements.md    # [新增] 本文件
│
│ ── 归档文件（不再使用）──
├── ws_bridge.py              # [归档] V2 Bridge
├── werewolf_agent.py         # [归档] V1 Agent
├── strategy/                 # [归档]
├── memory.py                 # [归档]
├── prompts/                  # [归档]
├── logger.py                 # [归档]
...
下表总结各问题在 SDK 集成 + 业务脚本方案下的修复状态：

| # | 问题 | 严重度 | SDK 修复 | 业务脚本修复 | 仍需单独修复 |
|---|------|:------:|:--------:|:----------:|:----------:|
| 1 | WebSocket 协议不匹配 | 🔴 致命 | ✅ | — | — |
| 2 | REST API 路径错误 | 🔴 致命 | ✅ | — | — |
| 3 | 认证方式不匹配 | 🔴 致命 | ✅ | — | — |
| 4 | Action 字段名不匹配 | 🔴 致命 | ✅ 字段名 | ✅ 值映射 | — |
| 5 | room_id / game_id 混淆 | 🟠 严重 | ✅ | — | — |
| 6 | 事件消息结构错误 | 🟠 严重 | ✅ | — | — |
| 7 | 事件类型不完整 | 🟠 严重 | 大部分 | — | BUG-01~04 |
| 8 | 女巫/猎人行动不完整 | 🟠 严重 | — | ✅ 命令定义 | BUG-01~02 回调 |
| 9 | JSON 格式耦合到 Prompt | 🟡 设计 | — | ✅ | — |
| 10 | 降级策略不完整 | 🟡 次要 | — | — | BUG-05 |
